...
public class PlayerScript : MonoBehaviour
{
    ...
    public Color hitTakenTint;
    public Color invincibleTint;
    ...
    int hsAirHit = Animator.StringToHash("Base Layer.HitAir"); //Renombrar esta animacion!
    #endregion
    ...
    void Start()
    {
        ...
        normalTint = _SpriteRenderer.color;
    }
    void LateUpdate()
    {
    ...
    public void SetAnimations()
    {
        ...
        animatorController.SetBool("hitTrigger", isHitTrigger); isHitTrigger = false;
    ...
    IEnumerator applyDamage(int damage, int knockBackDir)
    {
        isInvincible = true;
        isHitTrigger = true;
        health -= damage;
        ...
        if (!isGrounded)
        {
            rigBody2D.AddForce(new Vector2(1000 * knockBackDir, 1000));
        }
        _SpriteRenderer.color = hitTakenTint;
        yield return new WaitForSeconds(1);
        _SpriteRenderer.color = invincibleTint;
        while (!isGrounded) { yield return null; }
        yield return new WaitForSeconds(2);
        isInvincible = false;
        _SpriteRenderer.color = normalTint;
        yield break;
    }
